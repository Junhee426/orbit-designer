...
- 관측지별 통신 접속 시간표: 목표 충족 / 목표 미충족 / 가시 위성 없음, 시간 가중 평균·최저 처리량·최저 마진, 최장 연속 접속·목표 미충족 시간. 구간 시각을 눌러 지도로 이동하고 전체 구간 CSV를 내보냅니다.
...
4. **후보 비교**에서 같은 관측지·기간·목표로 6개 Walker 후보를 비교합니다. **이 후보 적용**은 해당 후보의 고도·위성 수와 비교 당시 관측지·RF·항법·기간 조건을 함께 적용합니다. 순간 민감도는 별도 영역입니다.
5. 유효한 설정은 이 브라우저에 자동 저장되며 다시 열면 복원됩니다. 지도 모드·관측지·선택 위성·경과 시간도 포함합니다. 잘못된 입력은 마지막 유효 설정을 덮어쓰지 않습니다. 공유 링크로 열면 공유 설정을 우선합니다. 저장소 접근이 차단되면 설정 파일 저장을 사용하세요. 분석 결과는 자동 저장하지 않으므로 JSON·CSV로 내보내거나 다시 분석하세요. PWA 설치 기능은 없습니다.
6. 지도에서 **한 단계**는 분석 시간 간격만큼 이동합니다. **30× / 120× / 600×**로 재생 속도를 바꾸고 **처음**으로 돌아갈 수 있습니다. 재생은 마지막 시각에서 멈춥니다.

## 지표와 모델 범위

통신 접속 구간은 같은 상태의 연속 표본 간격을 합칩니다. 접속 위성이 바뀌어도 목표를 계속 충족하면 같은 접속 구간입니다. **최장 목표 미충족**은 RF 목표 미달과 가시 위성이 없는 시간이 이어지는 경우를 합산하므로 기존 **최장 표본 단절**(기하 가시성 기준)과 구별합니다. 구간 경계는 표본 해상도이며 표본 사이 짧은 단절은 놓칠 수 있습니다. 구간 CSV는 선택 관측지의 모든 구간과 원래 분석 조건·메타데이터를 포함합니다. 설정을 바꾼 뒤 이전 결과에서 지도로 이동하는 기능은 비활성화됩니다.
...
- `analysis.js`, `analysis-worker.js`, `contact-windows.js`: 관측지별 기간 분석·접속 구간·후보 비교.
- `session.js`: 유효 설정의 브라우저 저장·복원.
