...
export function defaultScenario() {
  return { schema_version:SCHEMA_VERSION, name:'K-LEO 한국 통신망', configuration:structuredClone(DEFAULT_CONFIG),
    ...
    analysis:{duration_min:1440,step_sec:300}, display:{mode:'3d',earthStyle:'image',orbits:true,isl:false,heatmap:false,footprint:true,time_sec:0,selected_satellite:null,domain:'commNav',satShape:'circle',satSize:1,orbitWidth:1}, active_observer:'KOR:0' };
}
...
export function validateScenario(input,catalog) {
  ...
  keys(s.display,['mode','earthStyle','orbits','isl','heatmap','footprint','time_sec','selected_satellite','domain','satShape','satSize','orbitWidth'],'표시');
  ...
  if(!['circle','square','diamond'].includes(s.display.satShape)||!Number.isFinite(s.display.satSize)||s.display.satSize<0.5||s.display.satSize>3||!Number.isFinite(s.display.orbitWidth)||s.display.orbitWidth<0.5||s.display.orbitWidth>4) throw Error('위성 모양·크기 또는 궤도면 굵기 설정을 확인해 주세요.');
  return s;
}
