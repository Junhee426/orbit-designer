...
export function defaultScenario() {
  return { schema_version:SCHEMA_VERSION, name:'K-LEO 한국 통신망', configuration:structuredClone(DEFAULT_CONFIG),
    ...
    analysis:{duration_min:1440,step_sec:300}, display:{mode:'3D',earthStyle:'image',orbits:true,isl:false,heatmap:false,footprint:false,time_sec:0,selected_satellite:null}, active_observer:'KOR:0' };
}
...
export function validateScenario(input,catalog) {
  ...
  keys(s.display,['mode','earthStyle','orbits','isl','heatmap','footprint','time_sec','selected_satellite'],'표시');
  s.display.earthStyle??='image';s.display.time_sec??=0;s.display.selected_satellite??=null;
  ...
  if(!['2D','3D'].includes(s.display.mode)||!['image','outline'].includes(s.display.earthStyle)||['orbits','isl','heatmap','footprint'].some(k=>typeof s.display[k]!=='boolean')) throw Error('지도 표시 설정을 확인해 주세요.');
  return s;
}
