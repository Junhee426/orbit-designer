...
export function validateScenario(input,catalog) {
  ...
  keys(s.display,['mode','orbits','isl','heatmap','footprint','time_sec','selected_satellite'],'표시');
  s.display.time_sec??=0;s.display.selected_satellite??=null;
  ...
  if(!['2D','3D'].includes(s.display.mode)||['orbits','isl','heatmap','footprint'].some(k=>typeof s.display[k]!=='boolean')) throw Error('지도 표시 설정을 확인해 주세요.');
  return s;
}
