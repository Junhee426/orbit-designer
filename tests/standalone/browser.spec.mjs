...
test('standalone flow: local assets, J2, countries, analysis, exports, stale results and restored custom observer',async({page})=>{
  ...
  await expect(page.locator('[data-country]:checked')).toHaveCount(1);await expect(page.locator('[data-country="KOR"]')).toBeChecked();await expect(page.locator('[data-config="j2"]')).toBeChecked();
  ...
  await expect(page.locator('#station-table tbody tr')).toHaveCount(3);await expect(page.locator('#period-metrics')).toContainText('기하학적 가시율');
  ...
  await page.locator('[data-config="altitude"]').fill('888');await page.locator('[data-config="altitude"]').blur();await expect(page.locator('#analysis-note')).toContainText('이전 설정');
  ...
  await page.locator('[data-tab="design"]').click();await page.locator('#map-mode').selectOption('2D');await page.locator('#map-mode').selectOption('3D');await page.locator('#show-heatmap').check();await page.locator('#show-footprint').check();await page.locator('#route').click();await expect(page.locator('#route-result')).toContainText('ms');
...
test('multi-shell, TLE and cancellation stay usable',async({page})=>{
  ...
  await page.locator('[data-config="mode"]').selectOption('tle');await page.locator('#example-tle').click();await expect(page.locator('#orbit-note')).toContainText('SGP4');await expect(page.locator('#error')).toBeHidden();
  ...
  await page.locator('[data-config="mode"]').selectOption('walker');await page.locator('#duration').fill('1440');await page.locator('#step').fill('60');await page.locator('#run').click();await page.locator('#cancel').click();await expect(page.locator('#status')).toHaveText('계산을 취소했습니다.');await expect(page.locator('#run')).toBeEnabled();
...
test('mobile layout runs with locally served assets',async({page})=>{
  await page.setViewportSize({width:390,height:844});await page.goto('/');await expect(page.locator('#status')).toContainText('준비 완료');await page.locator('#toggle-settings').click();await page.locator('#duration').fill('5');await page.locator('#run').click();await expect(page.locator('#status')).toHaveText('분석 완료');
