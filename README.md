...
지도 위 **3D / 2D 지구본 / 2D 평면도** 전환 버튼으로 표시 방식을 선택할 수 있습니다. 2D 보기는 CesiumJS 없이 캔버스로 그리는 가벼운 지구본·평면도이며(사용자의 AI 위성 관제 실험실 프로젝트와 동일한 방식), 지구본은 드래그로 회전합니다. 위성 위치, 관측 도시, 선택 위성의 지상 궤적·풋프린트를 표시하지만 궤도선·ISL 연결·지상국 연결·히트맵은 3D 보기에서만 표시됩니다. 전환 버튼 자체는 Cesium 로딩 여부와 무관하게 항상 사용할 수 있지만, 위성 위치·지상 궤적은 현재 궤도 배치가 최소 한 번 적용(3D 지도 초기화 성공)된 뒤에 나타납니다.
